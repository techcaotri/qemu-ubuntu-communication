...
args=(
	...
	-mem-prealloc
	-rtc base=localtime
	-drive file=ubuntu-24-04-base_2.qcow2,if=virtio,format=qcow2,cache=writeback
	-enable-kvm
  ...
  -spice agent-mouse=off,addr=/tmp/${NETNAME}/spice_2.sock,unix=on,disable-ticketing=on,rendernode=${NV_RENDER}
  ...
  -netdev bridge,id=hn0,br=virbr0
	...
	-chardev socket,id=char0,path=/tmp/vhostqemu_ubuntu_24_04_base_2
...
sudo rm /tmp/vhostqemu_ubuntu_24_04_base_2
sudo /usr/lib/qemu/virtiofsd --socket-path=/tmp/vhostqemu_ubuntu_24_04_base_2 --socket-group=${USER} -o source=${HOME}/Downloads/ -o allow_direct_io -o cache=always &
